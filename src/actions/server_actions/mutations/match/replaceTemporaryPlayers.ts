...
interface ReplaceTemporaryPlayerResponse {
    success: boolean;
    message: string;
}

interface ReplaceTemporaryPlayerParams {
    matchIdFromParams: string;
    temporaryPlayerId: string;
    teamNumber: number;
}

export async function replaceTemporaryPlayer({
    matchIdFromParams,
    temporaryPlayerId,
    teamNumber
}: ReplaceTemporaryPlayerParams): Promise<ReplaceTemporaryPlayerResponse> {
    ...
    if (!matchIdFromParams || !temporaryPlayerId || !teamNumber) {
